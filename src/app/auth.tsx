    ...
    <AuthContext.Provider value={{ user }}>
      {user !== undefined && user !== null ? (
       ...
       {(username.length>=3 && available && image) && <img src={previewImage as string} alt='Preview' className="image-auth"  />}
        ...
        <div className="bg-transparent border-2 border-yellow-400 p-4 rounded-lg text-center shadow-md hover:shadow-yellow-400/50 transition-shadow duration-300 mb-10">
          <h1 className="text-3xl font-bold text-yellow-400">Enter The User Name</h1>
        </div>
        <input id="username" placeholder="enter here" className="text-black" required onChange={(e)=>setUsername(e.target.value)}/>
        <br/>
      <p >{(username.length>=3 && !available) && <button className="signin-button" onClick={()=>handleSignIn()}>{(!isLoading) ?'SignIn':'wait...'}</button>}</p>
